...
class ControllerPaymentOpayo extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/opayo', 'token=' . $this->session->data['token'], true)
		);
						
		$data['action'] = $this->url->link('payment/opayo', 'token=' . $this->session->data['token'], true);
		...
		if (!$data['setting']['cron']['url']) {
			$data['setting']['cron']['url'] = $catalog . 'index.php?route=payment/opayo/cron&token=' . $data['setting']['cron']['token'];
		}
		...
		$this->response->setOutput($this->load->view('payment/opayo/opayo', $data));
	}

	public function install() {
		$this->load->model('payment/opayo');
		
		$this->model_payment_opayo->install();
	}

	public function uninstall() {
		$this->load->model('payment/opayo');
		
		$this->model_payment_opayo->uninstall();
	}
	...
	public function order() {
		if ($this->config->get('opayo_status')) {
			...
			$opayo_order = $this->model_payment_opayo->getOrder($this->request->get['order_id']);
			...
			if (!empty($opayo_order)) {
				...
				$opayo_order['total_released'] = $this->model_payment_opayo->getTotalReleased($opayo_order['opayo_order_id']);
	...
	public function void() {
		...
		if (!empty($this->request->post['order_id'])) {
			...
			$opayo_order = $this->model_payment_opayo->getOrder($this->request->post['order_id']);

			$void_response = $this->model_payment_opayo->void($this->request->post['order_id']);

			$this->model_payment_opayo->log('Void result', $void_response);

			if (!empty($void_response) && $void_response['Status'] == 'OK') {
				$this->model_payment_opayo->addOrderTransaction($opayo_order['opayo_order_id'], 'void', 0.00);
				$this->model_payment_opayo->updateVoidStatus($opayo_order['opayo_order_id'], 1);
	...
	public function release() {
		...
		if (!empty($this->request->post['order_id']) && !empty($this->request->post['amount']) && $this->request->post['amount'] > 0) {
			...
			$release_response = $this->model_payment_opayo->release($this->request->post['order_id'], $this->request->post['amount']);

			$this->model_payment_opayo->log('Release result', $release_response);

			if (!empty($release_response) && $release_response['Status'] == 'OK') {
				$this->model_payment_opayo->addOrderTransaction($opayo_order['opayo_order_id'], 'payment', $this->request->post['amount']);

				$total_released = $this->model_payment_opayo->getTotalReleased($opayo_order['opayo_order_id']);

				if ($total_released >= $opayo_order['total'] || $opayo_order['settle_type'] == 0) {
					$this->model_payment_opayo->updateReleaseStatus($opayo_order['opayo_order_id'], 1);
					$release_status = 1;
	...
	public function rebate() {
		...
		if (!empty($this->request->post['order_id'])) {
			...
			$rebate_response = $this->model_payment_opayo->rebate($this->request->post['order_id'], $this->request->post['amount']);

			$this->model_payment_opayo->log('Rebate result', $rebate_response);

			if (!empty($rebate_response) && $rebate_response['Status'] == 'OK') {
				$this->model_payment_opayo->addOrderTransaction($opayo_order['opayo_order_id'], 'rebate', $this->request->post['amount'] * -1);

				$total_rebated = $this->model_payment_opayo->getTotalRebated($opayo_order['opayo_order_id']);
				$total_released = $this->model_payment_opayo->getTotalReleased($opayo_order['opayo_order_id']);

				if (($total_released <= 0) && ($opayo_order['release_status'] == 1)) {
					$this->model_payment_opayo->updateRebateStatus($opayo_order['opayo_order_id'], 1);
					$rebate_status = 1;
	...
	public function recurringButtons() {
		...
		if (!empty($this->request->get['order_recurring_id'])) {
			...
			if ($order_recurring_info) {
				...
				$data['info_url'] = str_replace('&amp;', '&', $this->url->link('payment/opayo/getRecurringInfo', 'token=' . $this->session->data['token'] . '&order_recurring_id=' . $data['order_recurring_id'], true));
				$data['enable_url'] = str_replace('&amp;', '&', $this->url->link('payment/opayo/enableRecurring', 'token=' . $this->session->data['token'], true));
				$data['disable_url'] = str_replace('&amp;', '&', $this->url->link('payment/opayo/disableRecurring', 'token=' . $this->session->data['token'], true));
				
				$content = $this->load->view('payment/opayo/recurring', $data);
			}
		}
	...
	public function enableRecurring() {
		if (!empty($this->request->post['order_recurring_id'])) {
			...
			$this->model_payment_opayo->editRecurringStatus($order_recurring_id, 1);
	...
	public function disableRecurring() {
		if (!empty($this->request->post['order_recurring_id'])) {
			...
			$this->model_payment_opayo->editRecurringStatus($order_recurring_id, 2);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/opayo')) {
