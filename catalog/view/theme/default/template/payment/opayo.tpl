...
function updateOpayo() {	
	...
	$('#opayo-form #opayo-button-delete-card').on('click', function () {
		if (confirm('<?php echo $text_confirm_delete; ?>')) {
			$.ajax({
				url: 'index.php?route=payment/opayo/deleteCard',
				type: 'post',
				...
				success: function (json) {
					...
					if (json['success']) {
						$('#opayo-form').load('index.php?route=payment/opayo/getForm #opayo-form >', function() {
							updateOpayo();
						});
					}
				}
	...
	$('#opayo-form #opayo-button-confirm').bind('click', function() {
		...
		$.ajax({
			type: 'post',
			url: 'index.php?route=payment/opayo/confirm',
